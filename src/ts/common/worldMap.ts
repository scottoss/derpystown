...
import { rain, splash } from './entities';
...
function removeWeatherEffects(map: WorldMap) {
	const effects = map.entities.filter(e => e.id === 0 && e.type === rain.type);
...
function addRainEffects(map: WorldMap) {
	forEachRegion(map, region => {
		if (region.x === 2 && region.y === 4) { // TEMP: testing
			const entity = rain((region.x + 1000) * REGION_SIZE, (region.y + 1000) * REGION_SIZE);
