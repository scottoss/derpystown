...
export function drawPonyEntity(batch: PaletteSpriteBatch, pony: Pony, drawOptions: DrawOptions) {
	...
	if (pony.batch !== undefined) {
        ...
        options.bodyScale = pony.bodyScale;
        options.bodyScaleNow = pony.bodyScaleNow;
...
export function drawPonyEntityLight(batch: SpriteBatch, pony: Pony, options: DrawOptions) {
    ...
    const scaling = pony.bodyScaleNow;

	if (draw) {
		batch.save();
		transformBatch(batch, pony);
        const { x, y } = getPonyHeadPosition(ponyState, 0, 0);
        batch.scale(scaling, scaling);
		batch.multiplyTransform(createHeadTransform(undefined, x, y, ponyState));
		...
		if (drawMagic) {
			...
			batch.drawImage(pony.magicColor, -1, -1, 2, 2, 30 - size / 2, 27 - size / 2, size, size);
		}
...
export function updatePonyEntity(pony: Pony, delta: number, gameTime: number, safe: boolean) {
	...
	// Transform
	if (pony.bodyScale != pony.bodyScaleNow) {
		if (isNaN(pony.bodyScaleNow)) pony.bodyScaleNow = 1;
		pony.bodyScaleNow += (pony.bodyScale - pony.bodyScaleNow) * .1;
		if (Math.abs(pony.bodyScaleNow - pony.bodyScale) < 1e-3) pony.bodyScaleNow = pony.bodyScale;
	}
	...
	if (pony.batch !== undefined) {
		...
		if (
			...
			options.toy !== pony.toy || options.bodyScaleNow !== pony.bodyScaleNow ||
	...
	const getTransformedBound = function (rekt: Rect) {
		rekt = rect(
			rekt.x * pony.bodyScaleNow, rekt.y * pony.bodyScaleNow,
			rekt.w * pony.bodyScaleNow, rekt.h * pony.bodyScaleNow);
		if (rekt.w < 0) rekt.w = -rekt.w, rekt.x -= rekt.w;
		if (rekt.h < 0) rekt.h = -rekt.h, rekt.y -= rekt.h;
		return rekt;
	}
	pony.bounds = getTransformedBound(pony.bounds);
	pony.interactBounds = getTransformedBound(pony.interactBounds);
	pony.lightBounds = getTransformedBound(pony.lightBounds);
	pony.lightSpriteBounds = getTransformedBound(pony.lightSpriteBounds);
}
