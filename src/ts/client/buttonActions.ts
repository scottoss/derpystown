...
const actionActions = [
	...
	actionButtonAction('scale-up', 'Scale up', Action.ScaleUp),
	actionButtonAction('scale-down', 'Scale down', Action.ScaleDown),
];
				...
				if (action.action === 'up' || action.action === 'down') {
					...
					buffer = drawCanvasCached(`action:${action.action}`, batch => {
						switch (action.action) {
							...
							case 'scale-up': {
								const ponyState = { ...createState(), animation: stand };
								drawPony(batch, actionPony, ponyState, 17, 13, defaultDrawPonyOptions());
								batch.drawSprite(sprites.arrow_up, BLACK, defaultPalette, 1, 2);
								break;
							}
							case 'scale-down': {
								const ponyState = { ...createState(), animation: stand };
								drawPony(batch, actionPony, ponyState, 17, 13, defaultDrawPonyOptions());
								batch.drawSprite(sprites.arrow_down, BLACK, defaultPalette, 1, 2);
								break;
							}
							default:
