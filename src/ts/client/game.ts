...
import {
	...
	REGION_SIZE, tileWidth, tileHeight, MAX_VELOCITY
...
export class PonyTownGame implements Game {
	...
	get volume() {
		return this.settings.browser.volume || 100;
	}
	get disableLighting() {
	...
	update(delta: number, now: number, last: number) {
		...
		if (player && this.loaded) {
			...
			let speed = flagsToSpeed(flags) * Math.sqrt(Math.abs(player.bodyScale || 1));
