...
import {
	...
	setEntityExpression, execAction, teleportTo, setBodyScale
} from './playerUtils';
...
export function createCommands(world: World): Command[] {
	const commands = compact([
		...
		command(['scale'], '/scale <number> - set character scaling', '', ({ }, client, message) => {
			if (typeof message === 'undefined' || message === '') message = '1';
			isNaN(parseFloat(message)) ? saySystem(client, 'Invalid number') : setBodyScale(client.pony, parseFloat(message));
		}),
