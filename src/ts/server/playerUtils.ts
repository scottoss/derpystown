...
import { EXPRESSION_TIMEOUT, DAY, FLY_DELAY, SECOND, PONY_TYPE, SCALING_LIMIT } from '../common/constants';
	...
	if (target) {
		...
		if (target.interact && (!target.interactRange || distance(pony, target) < target.interactRange)) {
			...
			switch (target.interactAction) {
				...
				case InteractAction.GiveCup: {
					if (client.pony.options!.hold === entities.cup.type) {
						unholdItem(pony);
					} else {
						holdItem(pony, entities.cup.type);
					}
					break;
				}
				case InteractAction.GiveLemonade: {
					if (client.pony.options!.hold === entities.lemonade.type) {
						unholdItem(pony);
					} else {
						holdItem(pony, entities.lemonade.type);
					}
					break;
				}
				default:
...
export function getBodyScale(entity: ServerEntity) {
	if (entity.options && typeof entity.options.bodyScale === 'number' && !isNaN(entity.options.bodyScale)) {
		return entity.options.bodyScale;
	}
	return 1;
}
export function setBodyScale(entity: ServerEntity, bodyScale: number) {
	if (entity.options) {
		if (bodyScale < SCALING_LIMIT[0]) bodyScale = SCALING_LIMIT[0];
		if (bodyScale > SCALING_LIMIT[1]) bodyScale = SCALING_LIMIT[1];
		if (entity.options.bodyScale !== bodyScale) updateEntityOptions(entity, { bodyScale });
	}
}
...
export function execAction(client: IClient, action: Action, settings: GameServerSettings) {
	switch (action) {
		...
		case Action.SwitchToTileTool:
			...
			break;
		case Action.ScaleUp:
			setBodyScale(client.pony, getBodyScale(client.pony) + 1);
			break;
		case Action.ScaleDown:
			setBodyScale(client.pony, getBodyScale(client.pony) - 1);
			break;
		default:
