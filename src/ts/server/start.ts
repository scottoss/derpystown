...
import { creatediscoMap } from './maps/discoMap';
import { createtardisMap } from './maps/tardisMap';
...
export function start(world: World, server: ServerConfig) {
	...
	world.maps.push(creatediscoMap(world));
	world.maps.push(createtardisMap(world));
	

	// custom map
	(DEVELOPMENT); { // remove `if` when you're ready to publish your map
