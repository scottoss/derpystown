...
import { EntityState, PonyOptions } from '../common/interfaces';
...
import { tileWidth, tileHeight, SECOND, MAX_VELOCITY } from '../common/constants';
...
export const createMove =
	(teleportCounter: CounterService<void>) =>
		(client: IClient, now: number, a: number, b: number, c: number, d: number, e: number, settings: GameServerSettings) => {
			...
			const options = pony.options as PonyOptions;
			const { x, y, dir, flags, time, camera } = decodeMovement(a, b, c, d, e);
			const v = dirToVector(dir);
			let speed = flagsToSpeed(flags) * Math.sqrt(Math.abs(options.bodyScale || 1));
			if (speed >= MAX_VELOCITY) speed = MAX_VELOCITY - 1e-4;
